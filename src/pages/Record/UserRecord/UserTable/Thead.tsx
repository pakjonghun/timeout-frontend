...
const Thead: React.FC<props> = ({ thead }) => {
  ...
  const { sortKey, sortValue } = useAppSelector((state) => state.record.userRecordTableHeadByRecent.sort);
  ...
  return (
    <thead className="text-gray-500 font-medium bg-gray-100">
      <tr className="whitespace-nowrap border-b-1">
        {thead.map(({ id, title, key }) => {
          if (title == '#' || key == 'select') {
            return (
              <th key={id} className="text-center py-3 px-6 sm:px-7 md:px-10">
                <input
